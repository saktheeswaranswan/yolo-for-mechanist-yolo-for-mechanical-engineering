...
def detect_objects(image, csv_writer, helmet_width_cm, helmet_height_cm, vernier_resolution_cm):
    ...
    for i in indices:
        ...
        if classes[class_ids[i]] == "helmet":
            ...
            width_estimate_cm = round((w / helmet_width) * helmet_width_cm, 5)
            height_estimate_cm = round((h / helmet_height) * helmet_height_cm, 5)
...
def perform_object_detection():
    cap = cv2.VideoCapture("ppetestg.mp4")
    ...
    with open("detection_results.csv", "w", newline="") as csvfile:
        ...
        # Input the vernier caliper's resolution options in centimeters
        vernier_resolution_options = [0.00001, 0.001, 0.0001]
        vernier_resolution_choice = int(input("Choose the vernier caliper's resolution (1: 0.00001 cm, 2: 0.001 cm, 3: 0.0001 cm): "))

        if vernier_resolution_choice < 1 or vernier_resolution_choice > 3:
            print("Invalid choice. Using default resolution of 0.00001 cm.")
            vernier_resolution_choice = 1

        vernier_resolution_cm = vernier_resolution_options[vernier_resolution_choice - 1]
