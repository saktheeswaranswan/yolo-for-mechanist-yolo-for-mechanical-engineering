...
def detect_objects(image, csv_writer, helmet_width, helmet_height):
    ...
    for i in indices:
        ...
        # Estimate the dimensions based on the given width and height estimates
        width_estimate = (w / helmet_width) * helmet_width
        height_estimate = (h / helmet_height) * helmet_height

        # Draw the bounding box and label on the image
        cv2.rectangle(image, (x, y), (x + w, y + h), (0, 255, 0), 2)
        label = f"{classes[class_ids[i]]}: {int(width_estimate)}x{int(height_estimate)}"
        cv2.putText(image, label, (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)

        # Save the detection result to the CSV file
        timestamp = time.strftime("%Y-%m-%d %H:%M:%S")
        csv_writer.writerow([timestamp, classes[class_ids[i]], int(width_estimate), int(height_estimate)])
...
def perform_object_detection():
    ...
    with open("detection_results.csv", "w", newline="") as csvfile:
        ...
        csv_writer.writerow(["Timestamp", "Class", "Width", "Height"])

        # Input the width and height estimates for the "helmet" class
        helmet_width = float(input("Enter the width estimate for the 'helmet' class: "))
        helmet_height = float(input("Enter the height estimate for the 'helmet' class: "))

        while True:
            ret, frame = cap.read()
            detect_objects(frame, csv_writer, helmet_width, helmet_height)
