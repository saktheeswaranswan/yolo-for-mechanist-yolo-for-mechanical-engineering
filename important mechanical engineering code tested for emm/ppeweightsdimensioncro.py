...
def detect_objects(image, csv_writer, helmet_width_cm, helmet_height_cm):
    ...
    for i in indices:
        ...
        if classes[class_ids[i]] == "helmet":
            ...
            width_estimate_cm = (w / helmet_width) * helmet_width_cm
            height_estimate_cm = (h / helmet_height) * helmet_height_cm

        # Draw the bounding box and label on the image
        cv2.rectangle(image, (x, y), (x + w, y + h), (0, 255, 0), 2)
        label = f"{classes[class_ids[i]]}: {int(width_estimate_cm)}x{int(height_estimate_cm)} cm"
        cv2.putText(image, label, (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)

        # Save the detection result to the CSV file
        timestamp = time.strftime("%Y-%m-%d %H:%M:%S")
        csv_writer.writerow([timestamp, classes[class_ids[i]], int(width_estimate_cm), int(height_estimate_cm)])
...
def perform_object_detection():
    ...
    with open("detection_results.csv", "w", newline="") as csvfile:
        ...
        helmet_width_cm = 21.7
        helmet_height_cm = 16

        while True:
            ret, frame = cap.read()
            detect_objects(frame, csv_writer, helmet_width_cm, helmet_height_cm)
